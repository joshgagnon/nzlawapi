...
def get_links(centre_id):
    with get_db().cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        query = """
            SELECT id, title, year, 'act' AS type, CONCAT('act_', id) AS name, RANDOM() AS y FROM acts
            WHERE id = %(id)s
            UNION ALL
            SELECT id, title, year, 'act' AS type, CONCAT('act_', id) AS name, RANDOM() AS y FROM acts a
            JOIN act_references f ON (f.target_id = a.id AND f.mapper = 'act')
            WHERE f.source_id = %(id)s
            UNION ALL
            SELECT id, title, year, 'regulation' AS type, CONCAT('regulation_', id) AS name, RANDOM() AS y FROM regulations r
        ...
        query = """
            SELECT CONCAT('act_', source_id) AS source_name, CONCAT(mapper, '_', target_id) AS target_name FROM act_references
            WHERE source_id IN %(source_acts)s
            AND target_id IN %(targets)s
            UNION ALL
            SELECT CONCAT('regulation_', source_id) AS source_name, CONCAT(mapper, '_', target_id) AS target_name FROM regulation_references
            ...
            try:
                ...
                for source in collection:
                    if source['name'] == link['source_name']:
                        try:
                            source['references'].append(target_idx)
                        except KeyError:
                            source['references'] = [target_idx]
                        break
            except ValueError:
