...
def run(db, config):
    ...
    with db.cursor(cursor_factory=extras.RealDictCursor, name="law_cursor") as cur:
        cur.execute("""SELECT id, title, document, type, subtype, number, date_terminated,
            ...
            date_gazetted, date_imprint, year, repealed, generation, children FROM latest_instruments""")
    ...
    with db.cursor(cursor_factory=extras.RealDictCursor, name="law_cursor") as cur:
        cur.execute("""SELECT i.id, neutral_citation, court, full_citation, parties::text, counsel,
            judgment, waistband, hearing, received, matter::text, charge, plea, bench, document, appeal_result::text, 1 as generation, 0 as children FROM cases i
